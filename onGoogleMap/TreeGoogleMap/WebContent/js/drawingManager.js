...
function queryResultDrawing(cArray){

	var map = reMap();

	drawingTool(cArray, map);
	...
	//alert(queryResult[0].level) : 1
	//alert(root[0].level) : 4
	...
	for(var i=0; i<queryResult.length; i++){
		...
		//search의 결과 leaf node의 rectangle들이 출력된다면,
		//alert으로 leaf node임을 알려주고 아무런 동작도 하지 않아야 한다.
		alertListener(queryResult[i], temp[i]);
	}
}
...
function queryListener(root, drawingManager){
	 ...
	 google.maps.event.addListener(drawingManager, 'overlaycomplete', function(e) {
		...
		google.maps.event.addListener(queryWindow, 'click', search(root, plevel, queryWindow));

		var temp = new Array();
		temp = queryResult;
		
		queryResultDrawing(temp);
    });
